...
ICSFileBuilder.prototype.toString = function () {
  ...
  for (var idx = 0; idx < this.events.length; idx++) {
    ...
    if (_event.busystatus) lines.push('X-MICROSOFT-CDO-BUSYSTATUS:' + _event.busystatus.toUpperCase())
...
ICSFileBuilder.prototype._checkAndBuildEventObject = function (rawEvent) {
  ...
  // Busy Status
  if (rawEvent.busystatus && allowedBusyStatuses.indexOf(rawEvent.busystatus.toUpperCase()) === -1) {
    return new Error('event.busystatus must be one of the following: ' + allowedBusyStatuses.join(', '));
  }
  event.busystatus = rawEvent.busystatus;
