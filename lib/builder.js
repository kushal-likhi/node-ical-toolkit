...
ICSFileBuilder.prototype.toString = function () {
  ...
  for (var idx = 0; idx < this.events.length; idx++) {
    ...
    if (_event.organizer) lines.push('ORGANIZER;CN="' + _event.organizer.name.replace(/"/g, '\\"') + '":mailto:' + _event.organizer.email);
    for (var ac = 0; ac < _event.attendees.length; ac++) {
      if (_event.attendees[ac] instanceof Error) {
        ...
        lines.push('ATTENDEE;ROLE=' + _event.attendees[ac].role + ';PARTSTAT=' + _event.attendees[ac].status + ';CN=' + _event.attendees[ac].name + ':MAILTO:' + _event.attendees[ac].email);
      }
    }
  ...
  (rawEvent.attendees || []).forEach(function (att) {
    if (att.name && att.email) {
      event.attendees.push({
        ...
        status: (att.status || 'TENTATIVE').toUpperCase()
      });
    } else {
